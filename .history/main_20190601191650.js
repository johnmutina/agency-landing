...
$(".form-submit").click(function(e){
    ...
    if (RegExp(/^[a-zA-Z]{3,}(([',. -][a-zA-Z ])?[a-zA-Z]*)*$/).test(name)) {
        ...
        if ($(".name-error").text() === "") {
            ...
            nameErr.text("Name should contain only letters and be long between 3 and 25 characters.");
    ...
    if (RegExp(/^[0-9]{9,12}$/).test(phone)) {
        console.log("YES")
        $(".phone-error").remove();
    } else {
        if ($(".phone-error").text() === "") {
            var emailErr = $("<div class='form-error phone-error'></div>");
            emailErr.text("Use only numbers, between 6 and 12 figures.");

            $("input[name=phone]").after(emailErr);
        }
    }
